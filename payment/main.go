...
func main() {
	cfg, err := config.Load("D:\\GolandProjects\\GoMall\\payment\\config.example.yaml")
	if err != nil {
		slog.Error("config加载失败", "error", err)
		panic(err)
	}
	slog.Info("config加载成功", "path", "payment/config.example.yaml")
	...
	paymentRepository := repository.NewPaymentRepository(mysqlDB)

	if err := paymentRepository.InitTable(); err != nil {
		slog.Error("init table error")
		panic(err)
	}

	paymentService := srv.NewPaymentDataService(paymentRepository)
	...
	service.Init()
	if err := pb.RegisterPaymentHandler(service.Server(), handler.NewPaymentHandler(paymentService)); err != nil {
