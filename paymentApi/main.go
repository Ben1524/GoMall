...
import (
	...
	"os/signal"
	"paymentApi/handler"
	payment "paymentApi/proto/payment"
	"paymentApi/router"
	"syscall"
	"time"
...
func main() {
	breakerName := "paymentApi"

	cfg, err := config.Load("D:\\GolandProjects\\GoMall\\paymentApi\\config.example.yaml")
	if err != nil {
		slog.Error("config加载失败", "error", err)
		panic(err)
	}
	slog.Info("config加载成功", "path", "paymentApi/config.example.yaml")
	...
	cartSrv := payment.NewPaymentService("go.micro.service.payment", service.Client())

	h := handler.NewPaymentApiHandler(cartSrv)
