...
func NewCartHandler(service service.ICartDataService) *Cart {
	return &Cart{
		CartDataService: service,
		// 定义tracer名称（建议包含服务名和组件名，确保唯一）
		tracer: otel.Tracer(tracerName, trace.WithInstrumentationVersion(version)),
	}
}
